...
<xsl:template match="testcase" mode="tableline">
    <xsl:variable name="cssclass">
        <xsl:choose>
            <xsl:when test="count(failure) &gt; 0 or count(error) &gt; 0">failure</xsl:when>
            <xsl:otherwise>success</xsl:otherwise>
        </xsl:choose>
    </xsl:variable>
    ...
    <div>
        ...
        <div class="metadata">
            ...
            <p>
                <b class="testname"><xsl:value-of select="@name" /></b>
            </p>
                
            <p>
                Executed in <xsl:value-of select="@time" /> seconds.
            </p>           
        </div>
        <div class="details">
...
<div class="failure">
    <p>
        <b class="errorMessage"><xsl:value-of select="@message" /></b>
    </p>
...
.testsuite .testcase:nth-of-type(even) {
